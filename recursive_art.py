...
def build_random_function(min_depth, max_depth):
    ...
    def build_func(depth):
        if depth == 0:
            ...
            depth -= 1
            random_func = random.choice(list_of_functions)
            if random_func not in list_funcs_two_inputs:
                return [random_func, build_func(depth)]
            else:
                return [random_func, build_func(depth), build_func(depth)]

    return build_func(depth)
...
def remap_interval(val,
    ...
    value_percentage = val/input_range
    output_range = output_interval_end - output_interval_start
    new_val = output_interval_start + (output_range*value_percentage)
    return new_val
